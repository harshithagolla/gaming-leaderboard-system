...
def insert_game_session(db: Session, user_id: int, score: int, game_mode: str = "solo"):
    ...
    db.add(gs)
    db.flush()  # ensure id assigned if needed
...
def get_top_n(db: Session, n: int = 10):
    q = text("""
    ...
    LIMIT :n
    """)
    res = db.execute(q, {"n": n}).fetchall()
    return [{"user_id": r[0], "username": r[1], "total_score": int(r[2])} for r in res]
